...
const programs = [
  {
    id: 1,
    title: "Divide Two 8-Bit Numbers",
    ...
    steps: [
      ...
      { instr: "MOV B, A", why: "Store the new (smaller) remainder back in B so it's safe for the next iteration. A might get used for other things.", reg: "B ← A (new remainder)", key: "Save updated remainder back to B before looping." },
      ...
      { instr: "MOV M, D", why: "M means 'memory at address HL'. So this stores quotient (D) into D002H. The [HL] notation means indirect memory access.", reg: "[D002H] ← D (quotient)", key: "MOV M, r always writes register r to address in HL." },
      ...
      { instr: "MOV M, B", why: "Store remainder (B) into D003H (current HL address). Both results are now saved in consecutive memory.", reg: "[D003H] ← B (remainder)", key: "Consecutive storage: quotient at D002H, remainder at D003H." },
      { instr: "RST 1", why: "Program ends here. RST 1 jumps to the monitor routine on the trainer kit which stops execution and lets you read the results.", reg: "STOP", key: "RST 1 = program terminator on 8085 trainer boards." },
      { instr: "ERROR: MVI A, FFH", why: "We reach here only if divisor was zero. FFH is our error code — a value that can't be a real quotient, so the user knows something went wrong.", reg: "A ← FFH", key: "FFH is used as error sentinel — signals invalid division." },
  ...
  {
    id: 2,
    title: "Count Even Bytes (7501H–75FFH)",
    addresses: "Block: 7501H to 75FFH | Result: 7600H",
    summary: "We scan every byte in the block. For each byte, we check its last bit (LSB). If LSB is 0, the number is even. We count all evens. That's it.",
    steps: [
      ...
      { instr: "JNZ ODD", why: "If result is NOT zero, LSB was 1, meaning ODD number. Skip the counter increment and jump to ODD label.", reg: "PC → ODD if Z=0", key: "JNZ = Jump if Not Zero. Odd numbers escape the counter." },
  ...
  {
    id: 3,
    title: "Count Occurrences of ADH (C000H–C00FH)",
    ...
    steps: [
      ...
      { instr: "RST 1", why: "Done. D000H holds how many times ADH appeared.", reg: "STOP", key: "" },
    ]
  },
  {
    id: 4,
    title: "2's Complement of 5 Numbers",
  ...
  {
    id: 5,
    title: "Reverse Block Transfer (D100H–D1FFH → D200H)",
    addresses: "Source: D100H to D1FFH | Destination: D200H onwards",
    ...
    steps: [
      ...
      { instr: "MVI C, 00H", why: "Initialize counter to 0. We use the 256-wrap trick: 0 → 1 → 2 → ... → 255 → wraps to 0 → JNZ fails → loop ends. That gives exactly 256 iterations.", reg: "C ← 00H", key: "00H counter with INR + JNZ gives 256 iterations (wraps around)." },
      ...
      { instr: "RST 1", why: "Block reversed and copied. D200H now has D1FFH's data, D201H has D1FEH's data, etc.", reg: "STOP", key: "" },
    ]
  },
  {
    id: 6,
    title: "Divide All Numbers by 2 (4000H–4009H)",
    addresses: "Block: 4000H to 4009H (10 numbers) | In-place",
    ...
    steps: [
      ...
      { instr: "STC", why: "SET Carry flag to 1. This seems backwards but it's step 1 of a two-step carry-clearing trick.", reg: "CY ← 1", key: "STC alone isn't useful. It's setup for CMC." },
  ...
  },
  {
    ...
    steps: [
      ...
      { instr: "DCR C", why: "We already consumed one element (the first one). Reduce counter so we compare the remaining (count-1) elements.", reg: "C ← C - 1", key: "First element is already in A as max. Only need count-1 more comparisons." },
      { instr: "JZ STORE", why: "If count was 1, C is now 0. Only one element in block — it's trivially the largest. Skip loop and store.", reg: "PC → STORE if Z=1", key: "Edge case: single element block. Handle it cleanly." },
  ...
  {
    id: 8,
    title: "Copy Block (C050H–C05FH → C070H)",
  ...
  },
  {
    ...
    steps: [
      ...
      { instr: "RLC", why: "Rotate Left through Carry: all bits shift left by 1, bit 7 wraps to bit 0. After 1 RLC: bits shifted left once.", reg: "A ← rotate left 1", key: "4 RLCs = 4 left rotations = nibble swap." },
      { instr: "RLC", why: "Second rotation.", reg: "A ← rotate left 2", key: "" },
      { instr: "RLC", why: "Third rotation.", reg: "A ← rotate left 3", key: "" },
      { instr: "RLC", why: "Fourth rotation. After 4 left rotations: upper nibble and lower nibble are now swapped. If original was ABH, now it's BAH.", reg: "A ← rotate left 4 = nibbles swapped", key: "4 rotations = 4 bits shifted = nibble exchange. ABCD EFGH becomes EFGH ABCD." },
  ...
  {
    id: 10,
    title: "Subroutine: Fill BBH/44H Alternately",
    addresses: "Locations: 2000H–2009H | Pattern: BBH, 44H, BBH, 44H...",
  ...
  {
    id: 11,
    title: "Separate Nibbles and Multiply",
    ...
    steps: [
      ...
      { instr: "MVI A, 00H", why: "Clear A to use as product accumulator. Multiplication result builds up here.", reg: "A ← 00H", key: "A = running product. Starts at zero." },
      { instr: "MOV D, C", why: "Copy lower nibble into D as our loop counter for addition.", reg: "D ← C (lower nibble)", key: "D drives the addition loop. C is preserved in case needed." },
      ...
      { instr: "DCR D", why: "Decrement addition counter.", reg: "D ← D - 1", key: "" },
  ...
  {
    id: 12,
    title: "Fill 20 Locations with BCH (8081H)",
  ...
  {
    id: 14,
    title: "Subtract and Store Positive Result",
    ...
    steps: [
      ...
      { instr: "SUB C", why: "A ← A - C. If [3600H] ≥ [3601H], result is positive, no borrow. If [3600H] < [3601H], result is negative and Carry flag sets (borrow).", reg: "A ← A - C", key: "Carry flag set = borrow occurred = result is negative." },
      ...
      { instr: "STORE: STA 3602H", why: "Store the positive result (either direct or magnitude) at 3602H.", reg: "[3602H] ← A", key: "Always positive — either original positive result or magnitude of negative." },
      { instr: "RST 1", why: "Done.", reg: "STOP", key: "" },
    ]
  },
  {
    id: 15,
    title: "Add Two BCD Numbers",
...
export default function App() {
  ...
  const selectProg = (i) => { setSelectedProg(i); setCurrentStep(0); setShowAll(false); };
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .prog-btn:hover { background: rgba(0,180,220,0.12) !important; border-color: #00b4dc !important; }
        .nav-btn:hover:not(:disabled) { background: rgba(0,212,255,0.15) !important; transform: scale(1.05); }
        .nav-btn:disabled { opacity: 0.25; cursor: not-allowed; }
        .step-pill { transition: all 0.2s; cursor: pointer; }
        .step-pill:hover { background: rgba(0,212,255,0.2) !important; }
        .toggle-btn:hover { background: rgba(255,165,0,0.15) !important; }
      `}</style>
      ...
      <div style={{
        ...
        <div style={{ color: "#2a5a8a", fontSize: "0.7rem" }}>◆</div>
        <div style={{ color: "#5a8ab0", fontSize: "0.75rem", letterSpacing: "1px" }}>INTERACTIVE WALKTHROUGH</div>
        <div style={{ marginLeft: "auto", color: "#2a5a8a", fontSize: "0.7rem", letterSpacing: "1px" }}>
          PROG {selectedProg + 1}/15 · STEP {currentStep + 1}/{totalSteps}
        </div>
      </div>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div style={{
          ...
          <div style={{ color: "#2a5a8a", fontSize: "0.6rem", letterSpacing: "2px", padding: "4px 8px 10px", textTransform: "uppercase" }}>Programs</div>
          {programs.map((p, i) => (
            <button
              key={p.id}
              className="prog-btn"
              ...
              style={{
                width: "100%",
                background: i === selectedProg ? "rgba(0,180,220,0.1)" : "transparent",
                border: `1px solid ${i === selectedProg ? "#00b4dc" : "#1a2e48"}`,
                borderRadius: "6px",
                padding: "8px 10px",
                marginBottom: "4px",
                cursor: "pointer",
              ...
              <div style={{ color: i === selectedProg ? "#00d4ff" : "#3a6a9a", fontSize: "0.6rem", letterSpacing: "1px", marginBottom: "2px" }}>
                PROG {String(p.id).padStart(2, "0")}
              </div>
              <div style={{ color: i === selectedProg ? "#c8d8f0" : "#4a7a9a", fontSize: "0.68rem", lineHeight: 1.3 }}>
                {p.title}
              </div>
            </button>
          ))}
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "16px" }}>
              <div>
                <div style={{ fontFamily: "'Orbitron', monospace", fontSize: "0.7rem", color: "#00d4ff", letterSpacing: "2px", marginBottom: "4px" }}>
                  PROGRAM {String(prog.id).padStart(2, "0")}
                </div>
                <div style={{ fontSize: "1rem", fontWeight: "bold", color: "#e8f4ff", marginBottom: "8px" }}>{prog.title}</div>
                <div style={{ fontSize: "0.68rem", color: "#2a7a9a", marginBottom: "8px", letterSpacing: "0.5px" }}>{prog.addresses}</div>
                <div style={{
              ...
              <button
                ...
                style={{
                  background: showAll ? "rgba(255,165,0,0.1)" : "transparent",
                  border: `1px solid ${showAll ? "#ffa500" : "#1a4a6a"}`,
                  color: showAll ? "#ffa500" : "#4a8ab0",
                  borderRadius: "6px",
                  padding: "8px 14px",
                  cursor: "pointer",
                  fontSize: "0.68rem",
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
              {prog.steps.map((s, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ color: "#1e4a6a", fontSize: "0.65rem", width: "20px", flexShrink: 0, paddingTop: "2px" }}>{String(i + 1).padStart(2, "0")}</div>
                  <div style={{ fontFamily: "'Share Tech Mono', monospace", color: "#39ff14", fontSize: "0.8rem", width: "180px", flexShrink: 0 }}>{s.instr}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: "0.75rem", color: "#8ab8d8", lineHeight: 1.6, marginBottom: s.key ? "4px" : 0 }}>{s.why}</div>
                    {s.key && <div style={{ fontSize: "0.68rem", color: "#ffa500", opacity: 0.8 }}>→ {s.key}</div>}
                  </div>
                  <div style={{ fontSize: "0.65rem", color: "#2a6a4a", fontFamily: "'Share Tech Mono', monospace", flexShrink: 0, width: "120px", textAlign: "right" }}>{s.reg}</div>
                </div>
              ))}
            </div>
          ) : (
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                ...
                {prog.steps.map((_, i) => (
                  <div
                    ...
                    style={{
                      ...
                      height: "22px",
                      borderRadius: "4px",
                      background: i === currentStep ? "#00d4ff" : i < currentStep ? "rgba(0,212,255,0.2)" : "rgba(30,60,90,0.4)",
                      border: `1px solid ${i === currentStep ? "#00d4ff" : i < currentStep ? "#1a6a9a" : "#1a3a5a"}`,
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      fontSize: "0.55rem",
                      color: i === currentStep ? "#000" : i < currentStep ? "#00a0c0" : "#2a5a7a",
              ...
              <div style={{ flex: 1, overflow: "auto", padding: "24px" }}>
                <div style={{
                  ...
                  {/* Step number */}
                  <div style={{ color: "#1e4a6a", fontSize: "0.65rem", letterSpacing: "2px", marginBottom: "8px" }}>
                  ...
                  <div style={{
                    ...
                    <div style={{ color: "#2a7a9a", fontSize: "0.6rem", letterSpacing: "2px", marginBottom: "8px" }}>INSTRUCTION</div>
                    <div style={{
                      fontFamily: "'Orbitron', monospace",
                      fontSize: "1.6rem",
                      fontWeight: 900,
                  ...
                  <div style={{
                    ...
                    <div style={{ color: "#1a6a4a", fontSize: "0.6rem", letterSpacing: "2px", flexShrink: 0 }}>EFFECT</div>
                    <div style={{ fontFamily: "'Share Tech Mono', monospace", color: "#39e88a", fontSize: "0.85rem" }}>{step.reg}</div>
                  </div>
                  ...
                  <div style={{
                    ...
                    <div style={{ color: "#1a5a9a", fontSize: "0.6rem", letterSpacing: "2px", marginBottom: "12px" }}>WHY THIS INSTRUCTION HERE</div>
                    <div style={{ fontSize: "0.88rem", color: "#a8c8e8", lineHeight: 1.8 }}>{step.why}</div>
                  </div>
                  ...
                  {step.key && (
                    <div style={{
                      ...
                      <div style={{ color: "#7a5a1a", fontSize: "0.6rem", letterSpacing: "2px", marginBottom: "6px" }}>KEY INSIGHT</div>
                      <div style={{ fontSize: "0.8rem", color: "#ffa500", lineHeight: 1.6 }}>{step.key}</div>
                    </div>
                  )}
                  ...
                  <div style={{ display: "flex", gap: "12px", alignItems: "center", justifyContent: "space-between" }}>
                    ...
                    <div style={{ color: "#1a3a5a", fontSize: "0.65rem" }}>
                    ...
                    <button
                      ...
                      style={{
                        background: currentStep === totalSteps - 1 ? "transparent" : "rgba(0,212,255,0.08)",
                        border: `1px solid ${currentStep === totalSteps - 1 ? "#1a4a7a" : "#00d4ff"}`,
                        color: currentStep === totalSteps - 1 ? "#2a6a9a" : "#00d4ff",
                        borderRadius: "8px",
                        padding: "12px 24px",
                        cursor: "pointer",
                        fontSize: "0.75rem",
